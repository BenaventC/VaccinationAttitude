...
Commençons par décrire la population.
...
On utilise les questions suivantes : 
...
On peut être encore plus précis, en étudiant les corrélation partielles pour détecter des relations spécifiques entre les indicateurs ( en se rappelant qu'un modèle factoriel se caractérise par des corrélations partielles qui tendent vers zero)
 * l'intention a tendance a être corrélé spécifiquement aux autres items mais plus particuilèement à la recommandation
...
## Sévérité du contact avec la maladie
 ...
 On mesure d'abord l'exposition à l'épidémie par une question binaire, l'exposition consuisant à une question auxilliaire sur la gravité de l'impact. On utilise cette variable en une échelle à 6 degrés comme proxy de l'expérience vécue.


```{r sev1}

ggplot(df,aes(x=Impactgrav))+geom_bar(aes(y = (..count..)/sum(..count..)), fill="peachpuff2") + scale_y_continuous(breaks = brks, labels = scales::percent(brks)) +coord_flip()+
  labs(title = "Répartition par Sévérité du contact covid", x=NULL, y="Proportion")
 ...
 * Dans quelle mesure votre vie a-t-elle été affectée par les contraintes engendrées par l'épidémie ?
...
C'est un système d'quation structurel dans lequel il y a trois variables purement exogènes :
...
Les deux variables endogènes sont l'attitude face à la vaccination ( intention +recommendation) et le jugement sur les politiques sanitaires. Ces derniers affectant l'attitude face au vaccins. Les variables médiatrices sont d'une part le sentiment d'être bien informé et  le degré avec lequel la vie est perturbée par l'épidémie et ses conséquences. 

L'attitude dépend surtout de la perception du risque, puis de l'intégration sociale et enfin dans une faibles mesure du jugement politique. La perception du risque dépend fortement de la confiance mais aussi du sentitiment d'adaptation à la situation. Ni 'expérience directe du convid , ni la perception de la gravité de la situation ne semble jouer, les variables deviennent completement endogènes.
